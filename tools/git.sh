...
git log                                                       # shows the log of commits
                                                                # git log by default uses less command so you can use these: f=next page, b=prev page, search=/<query>,
                                                                # n=next match, p=prev match, q=quit
...
git commit --amend                                            # combine staged changes with the previous commit, or edit the previous commit
                                                                # message without changing its snapshot
...
git config --global alias.<handle> <command>                  # add git aliases to speed up workflow , eg. if  handle is st and command is status then
                                                                # running git st would execute git status 
